    ...
    <div className="bg-gray-50 min-h-screen py-5">
      ...
      <section className="px-6 mb-12 text-center flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.isArray(challenges) && challenges.map((ch, index) => (
            <div key={ch._id || index} className="bg-white shadow-md p-5 rounded-xl border hover:shadow-lg transition">
              <img src={ch.imageUrl || "https://i.ibb.co/rR7R5sdW/ishikawa-noto-peninsula-135478.jpg"} alt={ch.title} className="w-full h-40 object-cover rounded-lg mb-3"/>
              <h3 className="text-xl font-semibold mb-2">{ch.title}</h3>
              <p className="text-gray-600 text-sm mb-1">Category: {ch.category}</p>
              <p className="text-gray-700 mb-2">{ch.description ? ch.description.slice(0, 80) : ""}...</p>
              <p className="text-gray-500 text-sm">Participants: {ch.participants || 0}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 mb-12 text-center flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.isArray(tips) && tips.map((tip, index) => (
            <div key={tip._id || index} className="bg-white shadow-md p-5 rounded-xl border hover:shadow-lg transition">
              <h3 className="text-lg font-semibold mb-2">{tip.title}</h3>
              <p className="text-gray-600 text-sm mb-1">By: {tip.authorName}</p>
              <p className="text-gray-700 text-sm">{tip.content ? tip.content.slice(0, 70) : ""}...</p>
              <p className="text-gray-500 text-sm">Upvotes: {tip.upvotes || 0}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 mb-12 text-center flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {Array.isArray(events) && events.map((ev, index) => (
            <div key={ev._id || index} className="bg-white shadow-md p-5 rounded-xl border hover:shadow-lg transition">
              <h3 className="text-lg font-semibold mb-1">{ev.title}</h3>
              <p className="text-gray-600 text-sm mb-1">Date: {ev.date ? new Date(ev.date).toLocaleDateString() : ""}</p>
              <p className="text-gray-700 text-sm">Location: {ev.location}</p>
              <p className="text-gray-500 text-sm">{ev.description ? ev.description.slice(0, 50) : ""}...</p>
            </div>
          ))}
        </div>
      </section>
